...
def order():
    return Order(
        line_items={SKU_1: LineItemInfo(quantity=2), SKU_2: LineItemInfo(quantity=2)}
    )
...
@pytest.mark.parametrize(
    ...
    (
        ...
        (
            [
                ShipmentUpdate(
                    tracking_code=TRACKING_CODE_1, skus=[SKU_1, SKU_1, SKU_2, SKU_2]
                )
            ],
            {TRACKING_CODE_1},
        ),
...
def test_order_add_shipments_success_all_shipped(
    shipment_updates, expected_tracking_codes, order
):
...
@pytest.mark.parametrize(
    ...
    (
        [
            ShipmentUpdate(
                tracking_code=TRACKING_CODE_1, skus=[SKU_1, SKU_2, INVALID_SKU]
            )
        ],
        [
            ShipmentUpdate(tracking_code=TRACKING_CODE_1, skus=[SKU_1, SKU_2]),
...
def test_order_add_shipments_fail_sku_not_present_in_line_items(
    shipment_updates, order
):
...
@pytest.mark.parametrize(
    ...
    (
        [
            ShipmentUpdate(
                tracking_code=TRACKING_CODE_1, skus=[SKU_1, SKU_1, SKU_2, SKU_2, SKU_2]
            )
        ],
        [
...
def test_order_add_shipments_fail_quantity_exceeded(shipment_updates, order):
    ...
    assert (
        f"Shipped quantity for SKU '{SKU_2}' exceeds order line item pending shipment quantity"
        in str(excinfo)
    )
    assert shipment_updates not in order.accepted_shipment_updates
